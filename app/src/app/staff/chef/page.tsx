...
import { api, usePoll, minutesAgo } from "@/lib/client";
...
export default function Chef() {
  ...
  const load = () => api<Kitchen>("/api/staff/kitchen").then((x) => { setD(x); setErr(""); }).catch((e) => { if (e.status === 401) router.push("/staff"); else setErr(e.message); });
  usePoll(load, 3000);
    ...
    <div className="aurora-d" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
      <div className="gd row" style={{ height: 68, borderRadius: 34, padding: "0 26px", justifyContent: "space-between" }}>
        ...
        <div className="row" style={{ gap: 20 }}>
          <div className="row" style={{ gap: 8, font: "700 13px var(--font-b)" }}><span style={{ width: 9, height: 9, borderRadius: "50%", background: err ? "#c62828" : "#3ddc97" }} />{err ? "OFFLINE" : "LIVE"}</div>
        ...
        {COL.map(([key, title, next, btn]) => (
          <div key={key} className="col">
            ...
            {d.tickets[key].map((o) => {
              ...
              return (
              <div key={o.id} className="gd r-xl" style={{ padding: "14px 16px", display: "flex", flexDirection: "column", gap: 4 }}>
                <div className="row" style={{ justifyContent: "space-between", alignItems: "baseline" }}><div style={{ font: "600 30px var(--font-h)" }}>Table {o.table || "?"}</div><div style={{ font: "700 13px var(--font-b)", color: mins >= 10 && key !== "ready" ? "#ffb4b4" : "rgba(255,255,255,.76)" }}>{minutesAgo(o.updatedAt)}</div></div>
