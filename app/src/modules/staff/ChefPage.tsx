...
export default function Chef() {
  ...
  // Same guard as the waiter screen: a signed-in waiter opening this URL directly would otherwise
  // see a fully working-looking kitchen screen and only find out via a 403 on the first button press.
  const load = () => api<Kitchen>("/api/staff/kitchen").then((x) => { if (x.me.role !== "chef") { router.push("/staff/waiter"); return; } setD(x); setErr(""); setLastUpdated(new Date()); }).catch((e) => { if (e.status === 401) router.push("/staff"); else setErr(e.message); });
  usePoll(load, 3000);
